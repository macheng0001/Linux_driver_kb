...
static void fake_gpiochip_set_value(struct gpio_chip *chip, unsigned offset,
                                    int value)
{
...
static int fake_gpiochip_direction_input(struct gpio_chip *chip,
                                         unsigned offset)
{
...
static int fake_gpiochip_direction_output(struct gpio_chip *chip,
...
static int fake_gpiochip_probe(struct platform_device *pdev)
{
    ...
    fake_chip =
        devm_kzalloc(&pdev->dev, sizeof(struct fake_gpiochip), GFP_KERNEL);
    if (!fake_chip) {
...
static const struct of_device_id fake_gpiochip_of_match[] = {
    {.compatible = "fake-gpio-chip"}, {/* Sentinel */}};
