...
#define DEVICE_NAME "nonblock_io"

struct nonblock_io_data {
...
{
    struct nonblock_io_data *p =
        container_of(inode->i_cdev, struct nonblock_io_data, cdev);
...
static ssize_t char_dev_read(struct file *file, char __user *buf, size_t len,
                             loff_t *loff_t)
{
    struct nonblock_io_data *p = file->private_data;
    int value;
...
{
    struct nonblock_io_data *p = (void *)data;
    int value;
...
static int nonblock_io_probe(struct platform_device *pdev)
{
    struct nonblock_io_data *p;
    int ret;

    printk("nonblock io probe\n");
    p = devm_kzalloc(&pdev->dev, sizeof(struct nonblock_io_data), GFP_KERNEL);
    if (!p) {
...
static int nonblock_io_remove(struct platform_device *pdev)
{
    struct nonblock_io_data *p;

    printk("block io remove\n");
...
static const struct of_device_id nonblock_io_of_match[] = {
    {.compatible = "nonblock-io"}, {/* Sentinel */}};

static struct platform_driver nonblock_io_driver = {
    .probe = nonblock_io_probe,
    .remove = nonblock_io_remove,
    .driver =
        {
            .name = "nonblock-io",
            .of_match_table = nonblock_io_of_match,
        },
};

module_platform_driver(nonblock_io_driver);

MODULE_AUTHOR("xiaoma <machengyuan@coinv.com>");
MODULE_DESCRIPTION("nonblock io test driver");
MODULE_VERSION("v1.0");
MODULE_LICENSE("GPL");
