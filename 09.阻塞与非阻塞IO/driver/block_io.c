...
#define DEVICE_NAME "block_io"

struct block_io_data {
...
{
    struct block_io_data *p =
        container_of(inode->i_cdev, struct block_io_data, cdev);
...
static ssize_t char_dev_read(struct file *file, char __user *buf, size_t len,
                             loff_t *loff_t)
{
    struct block_io_data *p = file->private_data;
    int value;
...
{
    struct block_io_data *p = (void *)data;
    int value;
...
static int block_io_probe(struct platform_device *pdev)
{
    struct block_io_data *p;
    int ret;

    printk("block io probe\n");
    p = devm_kzalloc(&pdev->dev, sizeof(struct block_io_data), GFP_KERNEL);
    if (!p) {
...
static int block_io_remove(struct platform_device *pdev)
{
    struct block_io_data *p;
...
static const struct of_device_id block_io_of_match[] = {
    {.compatible = "block-io"}, {/* Sentinel */}};

static struct platform_driver block_io_driver = {
    .probe = block_io_probe,
    .remove = block_io_remove,
    .driver =
        {
            .name = "block-io",
            .of_match_table = block_io_of_match,
        },
};

module_platform_driver(block_io_driver);

MODULE_AUTHOR("xiaoma <machengyuan@coinv.com>");
MODULE_DESCRIPTION("block io test driver");
MODULE_VERSION("v1.0");
MODULE_LICENSE("GPL");
