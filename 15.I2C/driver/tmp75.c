...
static int tmp75_write_reg(struct tmp75_priv *data, unsigned char reg,
                           unsigned char *buf, unsigned short len)
{
    unsigned char temp[1 + len];

    temp[0] = reg;
    memcpy(temp + 1, buf, len);

    return i2c_master_send(data->client, temp, len + 1);
}

static int tmp75_read_reg(struct tmp75_priv *data, unsigned char reg,
                          unsigned char *buf, unsigned short len)
{
    struct i2c_client *client = data->client;
    struct i2c_msg msg[2] = {{client->addr, 0, 1, &reg},
...
static int tmp75_set_resolution(struct tmp75_priv *data, resolution_cfg res_cfg)
{
    ...
    return tmp75_write_reg(data, REG_TMP75_CONF, buf, 1);
}
                                 ...
                                 resolution_cfg res_cfg, int *temperature)
{
    ...
    ret = tmp75_read_reg(data, REG_TMP75_TEMP, buf, 2);
    if (ret < 0) {
        return ret;
    }
