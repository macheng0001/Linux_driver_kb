...
static int virt_gpiochip_request(struct gpio_chip *chip, unsigned offset)
{
    printk("%s\n", __func__);
    return 0;
}

static void virt_gpiochip_free(struct gpio_chip *chip, unsigned offset)
{
    printk("%s\n", __func__);
}
...
static int virt_gpiochip_probe(struct platform_device *pdev)
{
    ...
    virt_gpiochip->gc.ngpio = 8;
    virt_gpiochip->gc.can_sleep = 0;
    virt_gpiochip->gc.request = virt_gpiochip_request;
    virt_gpiochip->gc.free = virt_gpiochip_free;
